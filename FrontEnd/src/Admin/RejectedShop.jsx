...
const RejectedShop = () => {
  const [shop,setshop] = useState([])
    ...
    const selshop = () => {
        axios.get("http://127.0.0.1:8000/Admin/rejectedshop/").then((response) => {
        ...
        <table className='table' id='tab'>
            ...
            {shop.map((shop, index) =>(
                <tr key={shop.id}>
                    ...
                    <td><i class="fa-solid fa-check edit-button" onClick={() => verify(shop.id, 1)}></i></td>
                </tr>
            ))}
        </table>
    </div>
